...
const LineCount: React.FC<IFilteredChartProps> = ({
  service,
  area,
  filterData,
  onChartMenuSelected,
  ...
  useEffect(() => {
    ...
    trackPromise<FilesPerDay[]>(service(filters, dataSource, columnName, dateColumn), area)
      .then((result) => {
        if (!isMounted()) return;
        setData(result);
      })
...
LineCount.displayName = "LineChartFilesCollected";
