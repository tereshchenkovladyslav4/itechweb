...
const BarOpenCaseSubtypeDaily: React.FC<IFilteredChartProps> = ({ service, filterData, area, onChartMenuSelected, chartIndex }: IFilteredChartProps) => {
    ...
    useEffect(() => {
        ...
        trackPromise<CaseStatusCountByDay[]>(service(filterData), area)
            .then((result) => {
                ...
                // get a distinct list of case subtypes
                // TBD... the enum names dont match the abb / descriptions in the DB..
                // const cols = Object.values(iTechDataCaseSubTypeEnum).filter(value => typeof value === 'string') as string[];
    ...
    return (
        <ChartMenuWrapper Chart={BarChart} callback={onChartMenuSelected} opt={{ area: area, dataKey: "name", data: data, dataValue: columns, seriesName: "caseStatusSubTypeAbb", title: "Open cases by subtype daily", chartIndex: chartIndex }} />
    );
}

BarOpenCaseSubtypeDaily.displayName = "BarOpenCaseSubtypeDaily";

export default BarOpenCaseSubtypeDaily;