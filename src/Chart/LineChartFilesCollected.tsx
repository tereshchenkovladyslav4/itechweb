...
const LineChartFilesCollected: React.FC<IFilteredChartProps> = ({
  service,
  area,
  filterData,
  onChartMenuSelected,
  ...
  useEffect(() => {
    ...
    trackPromise<FilesPerDay[]>(service(filters), area)
      .then((result) => {
        if (!isMounted()) return;
        setData(result);
      })
...
LineChartFilesCollected.displayName = "LineChartFilesCollected";

export default LineChartFilesCollected;
