...
const PieCaseStatus: React.FC<IFilteredChartProps> = ({
  service,
  area,
  filterData,
  onChartMenuSelected,
  ...
  useEffect(() => {
    ...
    trackPromise<CaseStatusCount[]>(service(filters), area)
      .then((result) => {
        if (!isMounted()) return;
        setData(result);
      })
    ...
    appliedModels,
    // JSON.stringify(
    //   selectors.getAllFiltersForDataSource(
    //     dataSource,
    //     TableEnum[dataSource as keyof typeof TableEnum],
    //     filterGroup
    //   )
    // ),
  ]);
...
PieCaseStatus.displayName = "PieCaseStatus";

export default PieCaseStatus;
