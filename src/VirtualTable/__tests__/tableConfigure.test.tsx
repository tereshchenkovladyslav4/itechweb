...
describe('<TableConfigure/> component', () => {
  it('renders the data source when state is configure grid', async () => {
    await act(async () => {
      const { tableServiceMock, resultsServiceMock, caseServiceMock, updateDataMock } = mockServices()
...
function mockServices() {
  ...
  const resultsServiceMock = {
    getAll: jest.fn(
      () => new Promise((resolve) => resolve([{ name: 'result 1', rowId: 10, forename: 'john', surname: 'doe' }])),
    ),
  } as unknown as typeof resultsService
  const caseServiceMock = {
    getAll: jest.fn(
      () => new Promise((resolve) => resolve([{ name: 'case 1', rowId: 1, forename: 'alex', surname: 'test' }])),
    ),
