
//*************************DO NOT MODIFY**************************
//
//THESE FILES ARE AUTOGENERATED WITH A TYPESCRIPT CODE GENERATION TOOL AND ANY MODIFICATIONS MADE HERE WILL BE LOST.
//PLEASE VISIT http://frhagn.github.io/Typewriter/ TO LEARN MORE ABOUT THIS VISUAL STUDIO EXTENSION
//
//*************************DO NOT MODIFY**************************

import { ITechControlCollectionType } from "./ITechControlCollectionType";



export class ITechControlRetentionPolicy {
    
    public rowId: number;
    public dateInserted: number | null;
    public dateModified: number | null;
    public dateArchived: number | null;
    public status: number[];
    public guid: number[];
    public signature: number[];
    public name: string;
    public retentionDuration: string;
    public age: number | null;
    public unitOfTime: string;
    public iTechControlCollectionTypeRowId: number | null;
    public isDefault: boolean | null;
    public iTechControlCollectionTypeRow: ITechControlCollectionType;
}


































