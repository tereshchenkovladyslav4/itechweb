...
import { applyTreeFiltersAction, removeTreeFiltersAction } from '../_context/actions/PageDataActions'
...
const TreeDisplay: React.FC<ITreeDisplayProps> = ({ config, area, service }): ReactElement => {
  ...
  const _onClickSelect = (node: any) => {
    ...
    if (node.type === 'Root') {
      ...
      if (!appliedTreeFilters || appliedTreeFilters.dataSources.every((x) => x.rowId === selectedTable.rowId)) {
        // replace / add whole filter
        dispatch(applyTreeFiltersAction(treeFilterModel, config.filterGroupColor))
      } else {
        // add to existing filters
        const otherDataSources = appliedTreeFilters.dataSources.filter((x) => x.rowId !== selectedTable.rowId)
  ...
  return <TreeDisplayRoot config={config} area={area} service={service} onClickSelect={_onClickSelect} />
}
...
const TreeDisplayRoot: React.FC<ITreeDisplayRootProps> = ({
  ...
  const _getData = (node: any) => trackPromise(service.get(_getKeyType(node)?.rowId, node.expressions), area)
  ...
  const _onClick = (e: any, node: any, select: boolean) => {
    ...
    return _getData(node).then((result) => {
      ...
      nodes.forEach((n: any) => _mapNode(n, keyTypeIndex, keyTypeIndex >= keyTypes.length, childNodes))
  ...
  const _loadTree = () => {
    roots.forEach((root: any) =>
      _getData(root).then((result) => {
        ...
        root.childNodes.map((n: any) => _mapNode(n, keyTypeIndex, keyTypeIndex >= keyTypes.length, childNodes))
